...
export function useCategory() {
    ...
    const updateCategory = async (categoryId: number, category: Tables<'category'>['Update']) => {
        const { error: updateError } = await supabase.from('category').update(category).eq('categoryid', categoryId);
        if (updateError) error.value = updateError.message;
        else await fetchCategories(); // Refresh list
    };

    const deleteCategory = async (categoryId: number) => {
        const { error: deleteError } = await supabase.from('category').delete().eq('categoryid', categoryId);
